...
import "../interfaces/IBitoroRebalancerCallback.sol";
import "./Account.sol";
import "./Storage.sol";
...
contract Liquidity is Storage, Account {
    ...
    function addLiquidity(
        ...
        require(blpPrice <= _storage.blpPriceUpperBound, "BPO"); // Blp Price is Out of range
        require(blpPrice >= _storage.blpPriceLowerBound, "BPO"); // Blp Price is Out of range
    ...
    /**
     * @notice Redeem bitoro token into original tokens.
     *
     *         Only strict stable coins and un-stable coins are supported.
     */
    function redeemBitoroToken(
        address trader,
        uint8 tokenId,
        uint96 bitoroTokenAmount // NOTE: OrderBook SHOULD transfer bitoroTokenAmount to LiquidityPool
    ) external onlyOrderBook {
        LibLiquidity.redeemBitoroToken(_storage, trader, tokenId, bitoroTokenAmount);
        emit RedeemBitoroToken(trader, tokenId, bitoroTokenAmount);
        _updateSequence();
    }
     ...
     *
     *       rebalancer must implement IBitoroRebalancerCallback.
     */
    function rebalance(
        address rebalancer,
        uint8 tokenId0,
