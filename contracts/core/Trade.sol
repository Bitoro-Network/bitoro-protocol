...
contract Trade is Storage, Account {
    ...
    function _realizeProfit(
        ...
        if (pnlUsd > 0) {
            ...
            // debt
            {
                uint96 bitoroTokenAmount = profitCollateral - spot;
                if (bitoroTokenAmount > 0) {
                    profitAsset.issueBitoroToken(trader, uint256(bitoroTokenAmount));
                    emit IssueBitoroToken(
