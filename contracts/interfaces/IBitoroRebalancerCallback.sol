 ...
 */
interface IBitoroRebalancerCallback {
    /**
     * @notice Rebalancer.bitoroRebalanceCallback is called when Brokers calls IOrderBook#fillRebalanceOrder, where
     *         Rebalancer is `msg.sender` of IOrderBook#placeRebalanceOrder.
     *
     *         Rebalancer will get token0 and send token1 back to `msg.sender`.
     */
    function bitoroRebalanceCallback(
        address token0,
        address token1,
        uint256 rawAmount0,
