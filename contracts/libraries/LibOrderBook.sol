...
library LibOrderBook {
    ...
    function redeemBitoroToken(
        ...
        address trader,
        uint8 tokenId,
        uint96 bitoroTokenAmount
    ) external {
        Asset memory asset = _storage.pool.getAssetInfo(tokenId);
        _transferIn(_storage, trader, asset.bitoroTokenAddress, address(_storage.pool), bitoroTokenAmount);
        _storage.pool.redeemBitoroToken(trader, tokenId, bitoroTokenAmount);
    }
