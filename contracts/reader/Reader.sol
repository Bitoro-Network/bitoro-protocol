...
contract Reader {
    ...
    function getChainStorage() public returns (ChainStorage memory chain) {
        ...
        // from assets
        address stableBitoroTokenAddress;
        ...
        for (uint256 i = 0; i < assetLength; i++) {
            ...
            if ((assets[i].flags & ASSET_IS_STABLE) != 0) {
                stableBitoroTokenAddress = assets[i].bitoroTokenAddress;
            } else {
                chain.assets[i].deduct = getDeduct(assets[i].bitoroTokenAddress);
            }
        }
    ...
    function getDeduct(address bitoroToken) internal view returns (uint256 deduct) {
        deduct = IERC20(bitoroToken).balanceOf(address(pool));
        for (uint256 i = 0; i < deductWhiteList.length; i++) {
            deduct += IERC20(bitoroToken).balanceOf(deductWhiteList[i]);
        }
        return deduct;
    }
    ...
    function _convertAssetStorage(Asset memory asset) internal pure returns (AssetStorage memory a) {
        ...
        a.bitoroTokenAddress = asset.bitoroTokenAddress;
        a.id = asset.id;
