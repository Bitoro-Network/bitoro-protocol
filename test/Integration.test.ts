...
describe("Integration", () => {
  ...
  beforeEach(async () => {
    ...
    bitoroWbtc = await createContract("BitoroToken")
    await bitoroWbtc.initialize("bitoroWbtc", "bitoroWbtc")
    await bitoroWbtc.transfer(pool.address, toWei(PreMinedTokenTotalSupply))
    ...
    // 1 = BTC
    // id, symbol, decimals, stable, token, bitoro
    await pool.addAsset(1, toBytes32("BTC"), 18, false, wbtc.address, bitoroWbtc.address)
    // id, symbol, imr, mmr, fee, fee, minBps, minTime, maxLong, maxShort, spotWeight
    ...
    // 2 = USDT
    // id, symbol, decimals, stable, token, bitoro
    await pool.addAsset(2, toBytes32("USDT"), 12, true, usdt.address, bitoroUsd.address)
    // id, symbol, imr, mmr, fee, fee, minBps, minTime, maxLong, maxShort, spotWeight
  ...
  it("+/-liquidity, +/-trade", async () => {
    ...
    }
    {
      ...
      expect(await blp.balanceOf(lp1.address)).to.equal(toWei("1098.89")) // (100 - fee) * 1000 / 1000
      expect(await blp.balanceOf(orderBook.address)).to.equal(toWei("0"))
      expect(await blp.balanceOf(pool.address)).to.equal(toWei("999999999999998901.11"))
  ...
  describe("bitoroToken", async () => {
    ...
    beforeEach(async () => {
      ...
      await wbtc.connect(lp1).approve(orderBook.address, toWei("100"))
      {
        ...
        expect(await wbtc.balanceOf(lp1.address)).to.equal(toWei("900"))
        expect(await wbtc.balanceOf(pool.address)).to.equal(toWei("100"))
        expect(await blp.balanceOf(lp1.address)).to.equal(toWei("199980")) // (100 - fee) * 2000 / 1
        expect(await blp.balanceOf(pool.address)).to.equal(toWei("999999999999800020"))
        const collateralInfo = await pool.getAssetInfo(1)
        expect(collateralInfo.spotLiquidity).to.equal(toWei("100")) // fee = 0.01
        expect(collateralInfo.collectedFee).to.equal(toWei("0.01"))
      }
      // withdraw 100 wbtc from LiquidityPool to LiquidityManager
      {
        ...
        expect(readerState.assets[1].deduct).to.eq(toWei("1000000000000000000")) // bitoroBTC supply = 0
        expect(readerState.stableDeduct).to.eq(toWei("1000000000000000000")) // bitoroUSD supply = 0
      }
    })

    it("close long, profit in bitoroBtc, and redeem bitoroBtc", async () => {
      // open long
      const longAccountId = assembleSubAccountId(trader1.address, 0, 1, true)
      await usdc.connect(trader1).approve(orderBook.address, toUnit("1000", 6))
      {
      ...
      // close long, profit in bitoroBtc, auto withdraw all
      {
        ...
        expect(await bitoroWbtc.balanceOf(trader1.address)).to.equal(toWei("0.046619047619047619")) // += pnl / mark - fee - [1].spotLiquidity
        const subAccount = await pool.getSubAccount(longAccountId)
        expect(subAccount.collateral).to.equal(toWei("0"))
        expect(subAccount.size).to.equal(toWei("0"))
        expect(subAccount.entryPrice).to.equal(toWei("0"))
        ...
        const readerState = await reader.callStatic.getChainStorage()
        expect(readerState.lpDeduct).to.eq(toWei("999999999999800020")) // lp supply = 199980
        expect(readerState.assets[1].deduct).to.eq(toWei("999999999999999999.953380952380952381")) // bitoroWbtc supply = 0.046619047619047619
        expect(readerState.stableDeduct).to.eq(toWei("1000000000000000000")) // bitoroUsd supply = 0
      }
      ...
      // redeem bitoroBtc
      await bitoroWbtc.connect(trader1).approve(orderBook.address, toWei("0.046619047619047619"))
      {
        await orderBook.connect(trader1).redeemBitoroToken(1, toWei("0.046619047619047619"))
        ...
        expect(await bitoroWbtc.balanceOf(trader1.address)).to.equal(toWei("0"))
        expect(await bitoroWbtc.balanceOf(pool.address)).to.equal(toWei("1000000000000000000"))
        const assetInfo = await pool.getAssetInfo(1)
        expect(assetInfo.spotLiquidity).to.equal(toWei("0.953380952380952381"))
        expect(assetInfo.collectedFee).to.equal(toWei("0.011")) // += fee
        const readerState = await reader.callStatic.getChainStorage()
        expect(readerState.lpDeduct).to.eq(toWei("999999999999800020")) // lp supply = 199980
        expect(readerState.assets[1].deduct).to.eq(toWei("1000000000000000000")) // bitoroWbtc supply = 0
        expect(readerState.stableDeduct).to.eq(toWei("1000000000000000000")) // bitoroUSD supply = 0
      }
    })

    it("close short (collateral usdc), profit in bitoroUsd, redeem to usdt", async () => {
      // +liq usdt
      ...
      // close short, profit in usdc (bitoroUsd), auto withdraw all
      {
        ...
        expect(await bitoroUsd.balanceOf(trader1.address)).to.equal(toWei("96.1")) // += pnl - fee - [0].spotLiquidity
        const subAccount = await pool.getSubAccount(shortAccountId)
        expect(subAccount.collateral).to.equal(toWei("0"))
        expect(subAccount.size).to.equal(toWei("0"))
        expect(subAccount.entryPrice).to.equal(toWei("0"))
        ...
        const readerState = await reader.callStatic.getChainStorage()
        expect(readerState.lpDeduct).to.eq(toWei("999999999999790021")) // lp supply = 209979
        expect(readerState.stableDeduct).to.eq(toWei("999999999999999903.9")) // bitoroUsd supply = 96.1
      }
      // redeem bitoroUsd into usdt
      await bitoroUsd.connect(trader1).approve(orderBook.address, toWei("96.1"))
      {
        await orderBook.connect(trader1).redeemBitoroToken(2, toWei("96.1"))
        expect(await usdc.balanceOf(trader1.address)).to.equal(toUnit("100000", 6))
        expect(await usdc.balanceOf(pool.address)).to.equal(toUnit("0", 6))
        ...
        expect(await bitoroUsd.balanceOf(trader1.address)).to.equal(toWei("0"))
        expect(await bitoroUsd.balanceOf(pool.address)).to.equal(toWei("1000000000000000000"))
        const collateralInfo = await pool.getAssetInfo(0)
        expect(collateralInfo.spotLiquidity).to.equal(toWei("0"))
        expect(collateralInfo.collectedFee).to.equal(toWei("3.9")) // += fee
        const collateral2Info = await pool.getAssetInfo(2)
  ...
  it("tp/sl strategy - open long", async () => {
    ...
    await wbtc.connect(trader1).approve(orderBook.address, toWei("10"))
    {
      ...
      const tx = orderBook
        .connect(trader1)
        .placePositionOrder3(longAccountId, toWei("10"), toWei("1"), toWei("3000"), 0, PositionOrderFlags.OpenPosition + PositionOrderFlags.TpSlStrategy, 86400 * 100, refCode, extra)
      await expect(tx)
        .to.emit(orderBook, "NewPositionOrder")
        .withArgs(longAccountId, 0, toWei("10"), toWei("1"), toWei("3000"), 0, PositionOrderFlags.OpenPosition + PositionOrderFlags.TpSlStrategy, 86400 * 100)
      await expect(tx)
        .to.emit(orderBook, "NewPositionOrderExtra")
        .withArgs(longAccountId, 0, toWei("10"), toWei("1"), toWei("3000"), 0, PositionOrderFlags.OpenPosition + PositionOrderFlags.TpSlStrategy, 86400 * 100, [extra.tpPrice, extra.slPrice, extra.tpslProfitTokenId, extra.tpslDeadline])
      expect(await wbtc.balanceOf(trader1.address)).to.equal(toWei("90"))
      expect(await wbtc.balanceOf(orderBook.address)).to.equal(toWei("10"))
    }
    {
    ...
    }
    {
      ...
      await expect(tx1)
        .to.emit(orderBook, "NewPositionOrder")
        .withArgs(longAccountId, 1, toWei("0"), toWei("1"), toWei("2001"), 0, PositionOrderFlags.WithdrawAllIfEmpty + PositionOrderFlags.ShouldReachMinProfit, 86400 * 101)
      await expect(tx1)
        .to.emit(orderBook, "NewPositionOrder")
    ...
    // close
    {
      ...
      await expect(orderBook.connect(broker).fillPositionOrder(2, toWei("1999"), toWei("1999"), toWei("1999"))) // pnl = -1
        ...
        .withArgs(1, OrderType.Position, [
          ...
          "0x000000000de0b6b3a764000000240000000d0d400002a3000000000000000001",
  ...
  it("tp/sl strategy - open short", async () => {
    ...
    await usdc.connect(trader1).approve(orderBook.address, toUnit("1000", 6))
    {
      ...
      const tx = orderBook
        .connect(trader1)
        .placePositionOrder3(shortAccountId, toUnit("1000", 6), toWei("1"), toWei("1000"), 0, PositionOrderFlags.OpenPosition + PositionOrderFlags.TpSlStrategy, 86400 * 100, refCode, extra)
      await expect(tx)
        .to.emit(orderBook, "NewPositionOrder")
        .withArgs(shortAccountId, 0, toUnit("1000", 6), toWei("1"), toWei("1000"), 0, PositionOrderFlags.OpenPosition + PositionOrderFlags.TpSlStrategy, 86400 * 100)
      await expect(tx)
        .to.emit(orderBook, "NewPositionOrderExtra")
        .withArgs(shortAccountId, 0, toUnit("1000", 6), toWei("1"), toWei("1000"), 0, PositionOrderFlags.OpenPosition + PositionOrderFlags.TpSlStrategy, 86400 * 100, [extra.tpPrice, extra.slPrice, extra.tpslProfitTokenId, extra.tpslDeadline])
      expect(await usdc.balanceOf(trader1.address)).to.equal(toUnit("99000", 6))
      expect(await usdt.balanceOf(trader1.address)).to.equal(toUnit("100000", 12))
    }
    {
    ...
    {
      const tx = orderBook.connect(broker).fillPositionOrder(0, toWei("1"), toWei("2000"), toWei("0"))
      await expect(tx)
        .to.emit(orderBook, "NewPositionOrder")
        .withArgs(shortAccountId, 1, toWei("0"), toWei("1"), toWei("1999"), 2, PositionOrderFlags.WithdrawAllIfEmpty + PositionOrderFlags.ShouldReachMinProfit, 86400 * 101)
      await expect(tx)
        .to.emit(orderBook, "NewPositionOrder")
    ...
    // close short with usdt (actually bitoroUsd)
    {
      ...
      expect(await usdt.balanceOf(trader1.address)).to.equal(toUnit("100000", 12)) // += 0
      expect(await bitoroUsd.balanceOf(trader1.address)).to.equal(toWei("98.1")) // += pnl - fee
      const subAccount = await pool.getSubAccount(shortAccountId)
      expect(subAccount.collateral).to.equal(toWei("0"))
      expect(subAccount.size).to.equal(toWei("0"))
      expect(subAccount.entryPrice).to.equal(toWei("0"))
      ...
      const readerState = await reader.callStatic.getChainStorage()
      expect(readerState.lpDeduct).to.eq(toWei("1000000000000000000"))
      expect(readerState.stableDeduct).to.eq(toWei("999999999999999901.9")) // bitoroUsd supply = 98.1
    }
    // another close should be canceled
    {
      const [orders, totalCount] = await orderBook.getOrders(0, 10)
  ...
  it("tp/sl strategy - close long", async () => {
    ...
    {
      await expect(
        orderBook.connect(trader1).placePositionOrder3(longAccountId, toWei("0"), toWei("1"), toWei("3000"), 0, PositionOrderFlags.TpSlStrategy, 86400 * 100, refCode, {
          tpPrice: toWei("2001"),
          slPrice: toWei("1999"),
          tpslProfitTokenId: 0,
          tpslDeadline: 86400 * 101,
      ...
      const tx = orderBook.connect(trader1).placePositionOrder3(longAccountId, toWei("0"), toWei("1"), toWei("0"), 0, PositionOrderFlags.TpSlStrategy, 86400 * 100, refCode, {
        tpPrice: toWei("2001"),
        slPrice: toWei("1999"),
        tpslProfitTokenId: 0,
        tpslDeadline: 86400 * 101,
  ...
  it("tp/sl strategy - liquidate long", async () => {
    ...
    await wbtc.connect(trader1).approve(orderBook.address, toWei("10"))
    {
      await expect(
        orderBook
          .connect(trader1)
          .placePositionOrder3(longAccountId, toWei("0.5"), toWei("1"), toWei("3000"), 0, PositionOrderFlags.OpenPosition + PositionOrderFlags.TpSlStrategy, 86400 * 100, refCode, {
            tpPrice: toWei("3000"),
            slPrice: toWei("1000"),
            tpslProfitTokenId: 0,
            tpslDeadline: 86400 * 101,
        ...
        .to.emit(orderBook, "NewPositionOrder")
        .withArgs(longAccountId, 0, toWei("0.5"), toWei("1"), toWei("3000"), 0, PositionOrderFlags.OpenPosition + PositionOrderFlags.TpSlStrategy, 86400 * 100)
    }
    {
      const [orders, totalCount] = await orderBook.getOrders(0, 10)
      expect(totalCount).to.eq(1)
