...
import { BlpToken, TestOrderBook, TestLiquidityPool, LiquidityManager, MockERC20, Reader } from "../typechain"
...
describe("IntegrationEth", () => {
  ...
  beforeEach(async () => {
    ...
    bitoroWeth = await createContract("BitoroToken")
    await bitoroWeth.initialize("bitoroWeth", "bitoroWeth")
    await bitoroWeth.transfer(pool.address, toWei(PreMinedTokenTotalSupply))

    // 0 = ETH
    // id, symbol, decimals, stable, token, bitoro
    await pool.addAsset(0, toBytes32("ETH"), 18, false, weth9.address, bitoroWeth.address)
    // id, symbol, imr, mmr, fee, fee, minBps, minTime, maxLong, maxShort, spotWeight
    await pool.setAssetParams(0, toBytes32("ETH"), rate("0.1"), rate("0.05"), rate("0.001"), rate("0.001"), rate("0.01"), 10, toWei("10000000"), toWei("10000000"), 2)
    // id, tradable, openable, shortable, useStable, enabled, strict, liq, halfSpread
    await pool.setAssetFlags(0, true, true, true, false, true, false, true, rate("0"))
  ...
  it("+/-liquidity, +/-trade", async () => {
    ...
    }
    {
      ...
      expect(await blp.balanceOf(lp1.address)).to.equal(toWei("99.99")) // (100 - fee) * 1000 / 1000
      expect(await blp.balanceOf(orderBook.address)).to.equal(toWei("0"))
      expect(await blp.balanceOf(pool.address)).to.equal(toWei("999999999999999900.01"))
    ...
    await blp.connect(lp1).approve(orderBook.address, toWei("1"))
    {
      ...
      expect(await blp.balanceOf(lp1.address)).to.equal(toWei("98.99"))
    ...
    }
    {
      ...
      expect(await blp.balanceOf(lp1.address)).to.equal(toWei("98.99"))
      expect(await blp.balanceOf(orderBook.address)).to.equal(toWei("0"))
      expect(await blp.balanceOf(pool.address)).to.equal(toWei("999999999999999901.01"))
      const collateralInfo = await pool.getAssetInfo(0)
  ...
  describe("bitoroToken", async () => {
    ...
    beforeEach(async () => {
      ...
      // withdraw 100 eth from LiquidityPool to LiquidityManager
      {
        ...
        expect(collateralInfo.collectedFee).to.equal(toWei("0.01"))
        const readerState = await reader.callStatic.getChainStorage()
        expect(readerState.assets[0].deduct).to.eq(toWei("1000000000000000000")) // bitoroETH supply = 0
        expect(readerState.lpDeduct).to.eq(toWei("999999999999800020")) // lp supply = 199980
      }
    })

    it("close long, profit in bitoroEth, and redeem bitoroEth", async () => {
      // open long
      const longAccountId = assembleSubAccountId(trader1.address, 0, 0, true)
      {
        await orderBook
      ...
      // close long, profit in bitoroBtc, auto withdraw all
      {
        ...
        expect(await bitoroWeth.balanceOf(trader1.address)).to.equal(toWei("0.045619047619047619")) // += pnl / mark - fee - [0].spotLiquidity
        const subAccount = await pool.getSubAccount(longAccountId)
        expect(subAccount.collateral).to.equal(toWei("0"))
        expect(subAccount.size).to.equal(toWei("0"))
        expect(subAccount.entryPrice).to.equal(toWei("0"))
        ...
        const readerState = await reader.callStatic.getChainStorage()
        expect(readerState.lpDeduct).to.eq(toWei("999999999999800020")) // lp supply = 199980
        expect(readerState.assets[0].deduct).to.eq(toWei("999999999999999999.954380952380952381")) // bitoroETH supply = 0.045619047619047619
      }
      ...
      // redeem bitoroBtc
      await bitoroWeth.connect(trader1).approve(orderBook.address, toWei("0.045619047619047619"))
      {
        const balance1 = await ethers.provider.getBalance(trader1.address)
        await orderBook.connect(trader1).redeemBitoroToken(0, toWei("0.045619047619047619"))
        const balance2 = await ethers.provider.getBalance(trader1.address)
