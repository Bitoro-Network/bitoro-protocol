...
describe("POL", () => {
  ...
  it("buyBITOROLP and withdraw", async () => {
    // distributor will transfer WETH into POL
    await weth9.deposit({ value: toWei("1") })
    await weth9.transfer(pol.address, toWei("1"))
    // buy
    {
      await expect(pol.buyBITOROLP(0, toWei("1"))).to.revertedWith("must be maintainer or owner")
      await expect(pol.connect(op).buyBITOROLP(0, toWei("1")))
