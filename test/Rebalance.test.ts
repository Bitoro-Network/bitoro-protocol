...
describe("Rebalance", () => {
  ...
  beforeEach(async () => {
    ...
    // 1 = ETH
    // id, symbol, decimals, stable, token, bitoro
    await pool.addAsset(1, toBytes32("ETH"), 18, false, weth9.address, bitoroWeth.address)
    // id, symbol, imr, mmr, fee, fee, minBps, minTime, maxLong, maxShort, spotWeight
    await pool.setAssetParams(1, toBytes32("ETH"), rate("0.1"), rate("0.05"), rate("0.001"), rate("0.001"), rate("0.01"), 10, toWei("10000000"), toWei("10000000"), 2)
    ...
    // 2 = USDC
    // id, symbol, decimals, stable, token, bitoro
    await pool.addAsset(2, toBytes32("USDC"), 6, true, usdc.address, bitoroUsd.address)
    // id, symbol, imr, mmr, fee, fee, minBps, minTime, maxLong, maxShort, spotWeight
    await pool.setAssetParams(2, toBytes32("USDC"), rate("0"), rate("0"), rate("0"), rate("0"), rate("0"), 0, toWei("0"), toWei("0"), 1)
    // id, tradable, openable, shortable, useStable, enabled, strict, liq, halfSpread
