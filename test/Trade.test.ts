...
describe("Trade", () => {
  ...
  let asset1: Contract
  let asset2: Contract
  let bitoroAsset0: Contract
  let bitoroAsset1: Contract
  ...
  beforeEach(async () => {
    ...
    bitoroAsset0 = await createContract("MockERC20", ["BITORO0", "BITORO0", 18])
    bitoroAsset1 = await createContract("MockERC20", ["BITORO1", "BITORO1", 18])
    await bitoroAsset0.mint(pool.address, toWei(PreMinedTokenTotalSupply))
    await bitoroAsset1.mint(pool.address, toWei(PreMinedTokenTotalSupply))

    // Asset 0 - strict stable
    // id, symbol, decimals, stable, token, bitoro
    await pool.addAsset(0, toBytes32("AST0"), 18, true, asset0.address, bitoroAsset0.address)
    ...
    // Asset 1 - position
    // id, symbol, decimals, stable, token, bitoro
    await pool.addAsset(1, toBytes32("AST1"), 18, false, asset1.address, bitoroAsset1.address)
    // id, symbol, imr, mmr, fee, fee, minBps, minTime, maxLong, maxShort, spotWeight
    ...
    // id, symbol, decimals, stable, token, bitoro
    await pool.addAsset(2, toBytes32("AST2"), 18, true, asset2.address, bitoroAsset0.address)
    // id, symbol, imr, mmr, fee, fee, minBps, minTime, maxLong, maxShort, spotWeight
  ...
  it("addLiquidity", async () => {
    ...
    // user 0 +liq
    {
      ...
      await expect(pool.addLiquidity(user0.address, 0, toWei("100"), toWei("1"), toWei("0.1"), current, target)).to.revertedWith("BPO")
      await expect(pool.addLiquidity(user0.address, 0, toWei("100"), toWei("1"), toWei("2.1"), current, target)).to.revertedWith("BPO")
  ...
  it("add/remove liquidity is forbidden", async () => {
    const current = toWei("29700")
    const target = toWei("29700")
  ...
  describe("bitoroToken", async () => {
    it("close long, profit in bitoroBtc, and redeem bitoroBtc", async () => {
      ...
      expect(subAccount.entryPrice).to.equal(toWei("0"))
      expect(subAccount.entryFunding).to.equal(toWei("0"))
      expect(await bitoroAsset1.balanceOf(user0.address)).to.equal(toWei("0.49")) // fee = 2, ((200 - 100) * 1 - fee) / 200
      expect(await asset1.balanceOf(user0.address)).to.equal(toWei("0"))
      ...
      // redeem bitoroBtc
      await pool.redeemBitoroToken(user0.address, 1, toWei("0.49"))
      expect(await asset1.balanceOf(user0.address)).to.equal(toWei("0.49"))
    })
    it("close short, profit in bitoroUsd, and redeem bitoroUsd", async () => {
      ...
      expect(subAccount.entryPrice).to.equal(toWei("0"))
      expect(subAccount.entryFunding).to.equal(toWei("0"))
      expect(await bitoroAsset0.balanceOf(user0.address)).to.equal(toWei("49.5")) // fee = 0.5, ((100 - 50) * 1 - fee) / 1
      expect(await asset1.balanceOf(user0.address)).to.equal(toWei("0"))

      // redeem bitoroUsd
      await expect(pool.redeemBitoroToken(user0.address, 2, toWei("98"))).to.revertedWith("STR") // only strict stable coin can be redeemed
      await expect(pool.redeemBitoroToken(user0.address, 0, toWei("98"))).to.revertedWith("LIQ")
      ...
      // redeem bitoroUsd
      await pool.redeemBitoroToken(user0.address, 0, toWei("98"))
